...
export function TutorialOverlay() {
  ...
  useEffect(() => {
    // Only show if tooltips are enabled and they haven't dismissed the tutorial before
    const hasSeenTutorial = localStorage.getItem('space_portfolio_tutorial');
    if (!hasSeenTutorial && preferences.showTooltips) {
      // Delay appearance slightly so they can take in the scene first
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-end justify-center pb-24 pointer-events-none">
      <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-2xl pointer-events-auto max-w-md w-full mx-4 animate-in slide-in-from-bottom-8 fade-in duration-700 relative">
        <button 
          onClick={dismiss}
